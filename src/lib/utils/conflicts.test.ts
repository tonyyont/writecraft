...
describe('detectOutlineDraftConflicts', () => {
  ...
  describe('section deletion detection', () => {
    it('should detect when a section with drafted content is deleted', () => {
      const previous = [createOutlinePrompt('1', 'Introduction'), createOutlinePrompt('2', 'Body')];
  ...
  describe('section modification detection', () => {
    ...
    it('should detect significant description changes', () => {
      ...
      const longDesc =
        'This is a much longer description that has been significantly expanded with additional detail and context.';

      const previous = [createOutlinePrompt('1', 'Section', shortDesc)];
      const current = [createOutlinePrompt('1', 'Section', longDesc)];
      const draft = createDraftContent([{ title: 'Section', content: 'Draft content here.' }]);
    ...
    it('should not flag minor description changes', () => {
      ...
      const draft = createDraftContent([{ title: 'Section', content: 'Draft content.' }]);

      const result = detectOutlineDraftConflicts(previous, current, draft);

      // Should have no conflicts for trivial description changes
      expect(result.conflicts.filter((c) => c.outlineChange.includes('description'))).toHaveLength(
        0
      );
    });
    ...
    it('should detect estimated word count changes', () => {
      ...
      const draft = createDraftContent([{ title: 'Section', content: 'Content here.' }]);
    ...
    it('should detect multiple modifications in one section', () => {
      const previous = [
        createOutlinePrompt('1', 'Old Title', 'Old description that is long enough.', 500),
      ];
      const current = [
        createOutlinePrompt(
          '1',
          'New Title',
          'New description that is different and long enough to be significant.',
          1000
        ),
      ];
      const draft = createDraftContent([{ title: 'Old Title', content: 'Some content.' }]);
  ...
  describe('section reordering detection', () => {
    it('should detect significant reordering (2+ positions)', () => {
      ...
      const draft = createDraftContent([{ title: 'Section D', content: 'Content for D.' }]);

      const result = detectOutlineDraftConflicts(previous, current, draft);

      expect(result.hasConflicts).toBe(true);
      const reorderedConflict = result.conflicts.find(
        (c) => c.conflictType === 'outline_reordered'
      );
    ...
    it('should not flag minor reordering (moved by 1 position)', () => {
      ...
      const draft = createDraftContent([{ title: 'Section B', content: 'Content for B.' }]);

      const result = detectOutlineDraftConflicts(previous, current, draft);

      const reorderedConflicts = result.conflicts.filter(
        (c) => c.conflictType === 'outline_reordered'
      );
  ...
  describe('title matching', () => {
    it('should match titles case-insensitively', () => {
      ...
      const current: OutlinePrompt[] = [];
      const draft = createDraftContent([{ title: 'INTRODUCTION', content: 'Content here.' }]);
    ...
    it('should match titles with extra whitespace', () => {
      ...
      const current: OutlinePrompt[] = [];
      const draft = createDraftContent([{ title: 'Section Title', content: 'Content here.' }]);
  ...
  describe('draft section extraction', () => {
    ...
    it('should handle sections with multi-line content', () => {
      ...
      const draft = createDraftContent([{ title: 'Section', content: multiLineContent }]);
  ...
  describe('no conflicts scenario', () => {
    it('should report no conflicts when outline changes do not affect drafted sections', () => {
      ...
      const draft = createDraftContent([{ title: 'Section 1', content: 'Content 1.' }]);
    ...
    it('should report no conflicts when drafted sections are unchanged', () => {
      ...
      const draft = createDraftContent([{ title: 'Section 1', content: 'Content.' }]);
